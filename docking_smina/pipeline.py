...
from docking_smina.workers import worker_dock_candidate, worker_dock_known
...
def run_baseline(
    ...
    logger.info("=== Step 1: Baseline & Cross-Docking (Smina) ===")

    thresholds: Dict[str, float] = {k: default_baseline for k in grids.keys()}
    ...
    with ProcessPoolExecutor(max_workers=n_cpu) as executor:
        ...
        for future in as_completed(futures):
            all_known_res.extend(future.result())

    if not all_known_res:
        logger.warning("No baseline result (Smina error?). Use default.")
        return thresholds
...
def run_candidates(
    ...
    logger.info(f"=== Step 2: Docking candidates (Smina, baseline: {thresholds}) ===")
    ...
    with ProcessPoolExecutor(max_workers=n_cpu) as executor:
        ...
        for future in as_completed(futures):
            ...
            if processed % 10 == 0:
                print(
                    f"Progress: {processed}/{len(cand_items)} | Hits: {hits_count}    ",
                    end="\r",
                )
