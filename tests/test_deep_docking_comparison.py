...
This test compares our GINE-based approach against the architecture described in:
  ...
  of Structure Based Drug Discovery." ACS Cent. Sci. 6, 939â€“949 (2020)
...
class TestDeepDockingComparison:
    ...
    def test_gnn_preserves_more_info_than_fingerprint(self, esol_data):
        ...
        gnn_features_per_mol = n_atoms * node_dim
    ...
    def test_print_comparison_summary(self, esol_data, trained_gnn, trained_mlp):
        ...
        print(f"  {'Uncertainty':<35} {'MC Dropout âś“':<18} {'None âś—':<18}")
        print(f"  {'Spearman Ď':<35} {gnn_rho:<18.4f} {mlp_rho:<18.4f}")
        print(f"  {'Enrichment @10%':<35} {gnn_ef:<18.2f} {mlp_ef:<18.2f}")
        print(f"  {'Selectivity scoring':<35} {'Built-in âś“':<18} {'Manual âś—':<18}")
        print(f"  {'Active Learning':<35} {'Built-in âś“':<18} {'None âś—':<18}")
        print(f"  {'Multi-GPU inference':<35} {'Built-in âś“':<18} {'None âś—':<18}")
