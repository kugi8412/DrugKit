...
being silently ignored — unless ``strict=False`` is passed, in which case
...
#: All supported bare atom symbols (aromatic lowercase + standard uppercase).
SUPPORTED_ATOMS: Final[frozenset[str]] = frozenset({
    "C", "N", "O", "S", "P", "F", "Cl", "Br", "I", "B", "H", "Si",
    "c", "n", "o", "s",
})

#: Atom symbols accepted inside bracket atoms — superset of SUPPORTED_ATOMS.
#: Includes metals and other elements that only appear in bracket form.
...
# Bracket atom regex
# ---------------------------------------------------------------------------
# Captures the contents of [...].
# Groups: isotope?, symbol, chirality?, hcount?, charge?, map?
...
def parse_bracket_atom(bracket_content: str) -> dict:
    ...
    * ``isotope`` — integer isotope (0 if absent; semantics are ignored downstream)
...
def tokenize_smiles(smiles: str, strict: bool = True) -> list[str | dict]:
    ...
    * A bracket-atom **dict** (see :func:`parse_bracket_atom`) for ``[...]``
    ...
    Args:
        ...
        strict: If ``True`` (default), raise on any unsupported feature.
            If ``False``, skip unrecognised characters with a warning —
    ...
    while pos < length:
        ...
        # -- Chirality (bare '@' outside brackets) -------------------------
        if char == "@":
            if smiles[pos: pos + 2] == "@@":
                tokens.append("@@")
                pos += 2
        ...
        # -- Digit '0' is not a valid ring index ---------------------------
        if char == "0":
            if strict:
                raise SMILESTokenizationError(
