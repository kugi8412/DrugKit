...
Provides:
  EncoderComparison   â€” runs both encoders on one SMILES, returns a diff report
  run_corpus          â€” runs over the full ESOL corpus and aggregates stats
...
The comparison distinguishes:
  ...
  - UNEXPECTED_DIFF: differs in a feature that should match â€” indicates a bug
...
pipeline and RDKit's rigorous chemistry engine):
  ...
  Atom features:
    ...
    hybridization (pos 31-36) Our bond-counting heuristic approximates SP/SP2/SP3.
                              RDKit uses a full valence model (e.g. amide N â†’ SP2,
                              sulfoxide S â†’ SP3D).
  ...
  Bond features:
    bond_type    (pos 0-3)   We infer AROMATIC from atom aromaticity; RDKit assigns
                              ...
                              mixed KekulĂ© notation the assignment can differ.
...
def compare_smiles(smiles: str) -> Optional[MoleculeDiff]:
    ...
    if len(rdkit_atoms) != len(g["atoms"]) or len(rdkit_bonds) != len(g["bonds"]):
        return None  # topology mismatch â€” exclude from comparison
...
def run_corpus(csv_path: Optional[Path] = None) -> CorpusStats:
    ...
    for smi in smiles_list:
        ...
        if diff is None:
            continue  # both parsers failed â€” skip
...
def print_report(stats: CorpusStats) -> None:
    ...
    print("=" * 60)
    print("ENCODER COMPARISON REPORT â€” ESOL corpus")
    print("=" * 60)
